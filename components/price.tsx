          ...
          {plans.map((item, idx) => (
            ...
            >
              <div className="p-4 py-8 space-y-4 border-b border-gray-700 md:p-8">
                ...
                <button className="px-3 py-3 rounded-lg w-full font-semibold text-sm duration-150  cyabg-n-500 hover:bg-cyan-600 active:bg-cyan-700">
                  Get Started
                </button>
              </div>
              <div>
                  ...
                  {item.features.map((featureItem, idx) => (
                    <li key={idx} className="flex items-center gap-5">
                      <svg
                        ...
                        className={`h-5 w-5 ${item.isMostPop ? "" : ""}`}
                        viewBox="0 0 20 20"
                        fill="currentColor"
                      >
                        <path
